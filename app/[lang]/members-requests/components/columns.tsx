 ...
 *
 * A factory rather than a constant: headers and `meta.label` are user-facing,
 * so the list is built against a language. See the note in the App Users
 * columns for why a module constant cannot hold translated strings.
 */
export function buildColumns(
  t: Translator
): ColumnDef<Features, MemberRequest, unknown>[] {
  return [
    {
      accessorKey: "id",
      header: () => <HeadCell icon={IdCard}>{t("requests.columns.request")}</HeadCell>,
      cell: ({ row }) => <IdCell value={row.original.id} />,
      size: 96,
      meta: { priority: 90, label: t("requests.columns.request") },
    },
    {
      accessorKey: "name",
      header: () => <HeadCell icon={User}>{t("requests.columns.applicant")}</HeadCell>,
      // The row's identity, so it carries the primary emphasis (§8.6).
      cell: ({ row }) => <TextCell value={row.original.name} emphasis="primary" />,
      size: 260,
      meta: { priority: 100, label: t("requests.columns.applicant") },
    },
    {
      accessorKey: "status",
      header: () => <HeadCell icon={CalendarDays}>{t("common.status")}</HeadCell>,
    ...
    },
    {
      ...
      size: 180,
      meta: { priority: 65, label: t("members.columns.phone") },
    },
    {
      id: "organization",
      ...
      size: 200,
      meta: { priority: 60, label: t("filters.attributes.organization") },
    },
    {
      accessorKey: "createdAt",
      header: () => (
        <HeadCell icon={CalendarDays}>{t("requests.columns.submitted")}</HeadCell>
      ),
      cell: ({ row }) => <DateCell value={row.original.createdAt} />,
      size: 190,
      meta: { priority: 70, label: t("requests.columns.submitted") },
    },
    {
      ...
      size: 190,
      meta: { priority: 50, label: t("members.columns.joinDate") },
    },
    {
      accessorKey: "note",
      header: () => (
        <HeadCell icon={MessageSquareText}>{t("requests.columns.note")}</HeadCell>
      ),
      // Only rejections carry one, so most rows are empty by design.
      cell: ({ row }) => <TextCell value={row.original.note} emphasis="quiet" />,
      enableSorting: false,
      size: 200,
      meta: { priority: 30, label: t("requests.columns.note") },
    },
  ]
}
