...
import { CreateBranchUserDialog } from "./components/user-dialog"
import { BranchUsersClient, BranchUsersCount } from "./components/users-client"

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations()
  return { title: t("branchUsers.metaTitle") }
}
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-muted">
              {t("branchUsers.countBefore")}
              <BranchUsersCount />
              {t("branchUsers.countAfter")}
            </p>
          </div>

          <CreateBranchUserDialog />
        </div>
