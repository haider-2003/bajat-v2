...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t("roles.branchMetaTitle") }
}

export default async function BranchRolesPage() {
  const t = await getTranslations()
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-muted">
              {t("roles.countBefore")}
              <RoleCount scope="branch" />
              {t("roles.branchCountAfter")}
            </p>
          </div>

          <CreateRoleDialog scope="branch" />
        </div>

        <RoleList scope="branch" />
      </div>
    </AppShell>
  )
}
