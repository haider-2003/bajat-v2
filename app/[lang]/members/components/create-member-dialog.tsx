...
function CreateMemberForm({ onCreated }: { onCreated: () => void }) {
  ...
  const formError = React.useMemo(() => {
    ...
    // `body.message` is the server's own wording and is passed through
    // untranslated — a client dictionary cannot localise an arbitrary string
    // it has never seen. Localising those is the backend's side of this.
    return (
      body?.message ??
      (error.response
  ...
  }, [createMember.isError, createMember.error, t])
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!name.trim()) found.name = t("members.form.nameRequired")
    if (!phone.trim()) found.phone = t("members.form.phoneRequired")
    else if (!wirePhone) found.phone = t("members.form.phoneInvalid")
    if (!chosenId) found.organizationIds = t("members.form.organizationRequired")
  ...
  return (
    ...
    >
      ...
      <DialogHeader>
        <DialogTitle>{t("members.new")}</DialogTitle>
        <DialogDescription>{t("members.newDescription")}</DialogDescription>
      </DialogHeader>
          ...
          <Field label={t("auth.phoneLabel")} error={errors.phone}>
            {(control) => (
              <InputGroup className="h-11 md:h-9">
                {/* `dir="ltr"`: "+964" is a Latin token whose sign leads it.
                    In an RTL row the browser would otherwise move the `+` to
                    the far end and render it as "964+". */}
                <InputGroupAddon
                  dir="ltr"
      ...
      <DialogFooter>
        ...
        <Button type="submit" className={ACTION} disabled={submitting}>
          ...
          {submitting ? t("common.creating") : t("members.create")}
        </Button>
      </DialogFooter>
    </DialogContent>
  )
