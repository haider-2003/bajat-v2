...
import { BlackListClient, BlackListCount } from "./components/black-list-client"
import { CreateEntryDialog } from "./components/create-entry-dialog"

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations()
  return { title: t("blackList.metaTitle") }
}
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-muted">
              {t("blackList.countBefore")}
              <BlackListCount />
              {t("blackList.countAfter")}
            </p>
          </div>
