 ...
 * ### Two views, not three
 *
 * There is no board: `isEnabled` is close to a delete, not a status to group by.
 *
 * ### What it does not filter on
 *
 * `/organization` reads `search`, `name`, `website`, `description` and the two
 * date ranges. `name` / `website` / `description` are what `search` already
 * covers, and `updated_at_range` answers an audit question this list is not
 * opened for — so the toolbar keeps `search` and the created-at window.
 */

/** Typing shouldn't fire a request per keystroke. */
const SEARCH_DEBOUNCE_MS = 300

/** The unfiltered total, pinned to the table's first cache entry. */
const COUNT_QUERY = { page: 1, pageSize: DEFAULT_PAGE_SIZE } as const

export function OrganizationsCount() {
  const query = useGetOrganizations(COUNT_QUERY)
...
export function OrganizationsClient() {
  ...
  const shown = useRecordSheet<Organization>()
  ...
  const editingOrganization = React.useMemo(() => {
    ...
    return data.find((organization) => organization.id === editing.id) ?? editing
  }, [data, editing])

  const deletingOrganization = React.useMemo(() => {
    if (!deleting) return null
    return data.find((organization) => organization.id === deleting.id) ?? deleting
  }, [data, deleting])

  const pageInfo = React.useMemo(
    () => readPageInfo(organizationsQuery.data?.data),
    [organizationsQuery.data]
  )
  ...
  const pagination = (
    <Pagination
      ...
      busy={organizationsQuery.isFetching}
  ...
  const emptyTitle = filtered
    ? t("organizations.emptyFiltered")
    : t("organizations.emptyTitle")
  const emptyHint = filtered ? undefined : t("organizations.emptyHint")
      ...
      <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-center">
        ...
        <div className="flex w-full flex-wrap items-center gap-2 lg:ms-auto lg:w-auto">
          <TextFilter
            ...
            label={t("organizations.searchLabel")}
