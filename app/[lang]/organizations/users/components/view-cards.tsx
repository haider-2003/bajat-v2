  ...
  return (
    <article className="flex flex-col rounded-xl border border-border bg-surface p-5">
      ...
      <dl className="mt-4 flex flex-col gap-2 border-t border-border-subtle pt-4">
        ...
        <Detail
          label={t("organizationUsers.columns.nodes")}
          value={
            nodes.length > 0
        ...
        />
        <Detail
          label={t("organizationUsers.columns.twoFactor")}
          value={
            user.tfaEnabled
...
export function CardView({
  ...
  table: TanTable<Features, OrganizationUser>
  onEdit: (user: OrganizationUser) => void
  onResetMfa: (user: OrganizationUser) => void
  onDelete: (user: OrganizationUser) => void
  emptyTitle: string
