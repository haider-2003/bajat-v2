...
import { CreateOrganizationUserDialog } from "./components/user-dialog"
import {
  OrganizationUsersClient,
  OrganizationUsersCount,
} from "./components/users-client"

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations()
  return { title: t("organizationUsers.metaTitle") }
}
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[13px] text-text-muted">
              {t("organizationUsers.countBefore")}
              <OrganizationUsersCount />
              {t("organizationUsers.countAfter")}
            </p>
          </div>
