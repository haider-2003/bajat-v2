...
import { useGetUsers, useResetUserMfa } from "@/features/users/api"
...
import { AdminActions, buildColumns } from "./columns"
import { DeleteAdminDialog } from "./delete-admin-dialog"
import { EditAdminDialog } from "./admin-dialog"
import { CardView } from "./view-cards"

/**
 * Admins — view switcher + toolbar.
 *
 * Same architecture as the other list screens. `/user` reads `search`, `name`,
 * `phone` and the two date ranges; there is no organization here to filter on.
 * The whole `/management` section only renders for `user.type === "admin"`
 * (components/layout/sidebar-nav-data.ts).
 */

const SEARCH_DEBOUNCE_MS = 300
const COUNT_QUERY = { page: 1, pageSize: DEFAULT_PAGE_SIZE } as const

export function AdminsCount() {
  const query = useGetUsers(COUNT_QUERY)
...
export function AdminsClient() {
  ...
  const shown = useRecordSheet<User>()
  ...
  const resetMfa = useResetUserMfa({ onSuccess: () => setResetting(null) })
  ...
  const emptyTitle = filtered
    ? t("admins.emptyFiltered")
    : t("admins.emptyTitle")
  const emptyHint = filtered ? undefined : t("admins.emptyHint")
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ResetMfaDialog
        ...
        copy={{
          title: t("admins.resetMfaTitle"),
          description: t("admins.resetMfaDescription"),
          note: t("admins.resetMfaNote"),
          action: t("admins.resetMfaAction"),
          pending: t("admins.resetMfaPending"),
          failed: t("admins.resetMfaFailed"),
        }}
      />
      <DeleteAdminDialog
        user={deletingUser}
