...
const STATUS_OPTIONS = PRINT_QUEUE_STATUSES.map((status: StatusName) => ({
...
export function PrinterClient() {
  ...
  // Memoized on the handler, which is itself stable: a fresh column array every
  // render would remount every cell — including the ones with a status change
  // in flight.
  const openPreview = React.useCallback((card: IDCard) => setPreviewed(card), [])
  const columns = React.useMemo(() => createColumns(openPreview), [openPreview])
  ...
  const activeFilters: ActiveFilter[] = [
    ...statusFilter.map((key) => ({
      key: "status-" + key,
      attribute: "Stage",
      value: STATUS_OPTIONS.find((option) => option.key === key)?.label ?? key,
      ...
      ? [
          {
            key: "created",
            attribute: "Created",
            value: describeRange(createdFrom, createdTo),
            onRemove: () => {
              setCreatedFrom("")
              setCreatedTo("")
            },
    ...
    ...(movedFrom || movedTo
      ? [
          {
            key: "moved",
            attribute: "Last moved",
            value: describeRange(movedFrom, movedTo),
            onRemove: () => {
              setMovedFrom("")
              setMovedTo("")
            },
      ...
      {idsQuery.isPending ? (
        ...
        <>
          {effectiveView === "table" && (
            <TableView
              table={table}
              emptyTitle="Nothing in the queue matches these filters"
              footer={pagination}
            />
          )}
