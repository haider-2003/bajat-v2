  ...
  if (cards.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-surface py-16">
        <EmptyState title="Nothing in the queue matches these filters" />
      </div>
    )
  }
  ...
  return (
    ...
    <article className="flex flex-col rounded-xl border border-border bg-surface p-5">
      ...
      <dl className="mt-4 flex flex-col gap-2 border-t border-border-subtle pt-4">
        <Detail label="Organization" value={formatText(card.organization?.name)} />
        <Detail label="Template" value={formatText(card.template?.title)} />
        <Detail label="Last moved" value={formatDate(card.updatedAt)} />
      </dl>

      {/* Footer — the card id, and the same two actions the table row has. */}
      <div className="mt-4 flex items-center justify-between gap-2 border-t border-border-subtle pt-3">
        ...
        <PrintActions card={card} onPreview={onPreview} />
      </div>
    </article>
  )
}
