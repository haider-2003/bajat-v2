...
export function RequestsClient() {
  ...
  /** Organization id as a string — `""` means every organization. */
   ...
   * for a single tick, and the date sends **`created_at_range[]`** carrying one
   * ISO instant. See docs/filtering-sorting-pagination.md §6.
   */
  const filter = React.useMemo(
    ...
    [search, phone, statusFilter, organizationId, createdAt]
  )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {requestsQuery.isPending ? (
        ...
        <>
          {effectiveView === "table" && (
            <TableView table={table} footer={pagination} />
          )}
