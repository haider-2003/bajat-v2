 ...
 */
export const columns: ColumnDef<Features, Member, unknown>[] = [
  {
    accessorKey: "id",
    header: () => <HeadCell icon={IdCard}>Member</HeadCell>,
    cell: ({ row }) => <IdCell value={row.original.id} />,
    size: 96,
    meta: { priority: 90, label: "Member" },
  },
  {
    accessorKey: "name",
    header: () => <HeadCell icon={User}>Name</HeadCell>,
    // The row's identity, so it carries the primary emphasis (§8.6).
    cell: ({ row }) => <TextCell value={row.original.name} emphasis="primary" />,
    size: 260,
    meta: { priority: 100, label: "Name" },
  },
  {
    accessorKey: "phone",
  ...
  },
  {
    ...
    size: 190,
    meta: { priority: 60, label: "Join date" },
  },
  {
    accessorKey: "createdAt",
    header: () => <HeadCell icon={CalendarDays}>Added</HeadCell>,
    cell: ({ row }) => <DateCell value={row.original.createdAt} />,
    size: 190,
    meta: { priority: 50, label: "Added" },
  },
]
